...
export default function AdminPanel() {
  ...
  return <DashboardLayout><div className="mx-auto max-w-6xl pb-16"><div className="flex flex-col gap-6 border-b border-white/10 pb-7 sm:flex-row sm:items-end sm:justify-between"><div><p className="font-mono text-[10px] font-semibold uppercase tracking-[.16em] text-[#b98cff]">Скрытая панель владельца</p><h1 className="mt-3 font-display text-[clamp(2.2rem,5vw,4.5rem)] font-black uppercase leading-[.84] tracking-[-.1em] text-white">{title}<br />студия.</h1><p className="mt-4 max-w-xl text-sm leading-6 text-[#aaa2b6]">Контент, медиафайлы и входящие заявки собраны в одном защищённом пространстве. Опубликованные изменения сразу используются на сайте.</p></div><div className="flex flex-wrap gap-3"><a href="/" target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-full border border-white/15 px-4 py-3 font-mono text-[10px] font-semibold uppercase tracking-[.12em] text-white">Открыть сайт <Globe2 className="h-4 w-4" /></a><button disabled={saveMutation.isPending || contentQuery.isLoading} onClick={() => saveMutation.mutate(draft)} className="inline-flex items-center gap-2 rounded-full bg-[#b98cff] px-5 py-3 font-mono text-[10px] font-bold uppercase tracking-[.12em] text-[#160e22] disabled:opacity-60">{saveMutation.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />}Сохранить текст</button></div></div>
    ...
    {contentQuery.isLoading ? <div className="grid min-h-64 place-items-center"><Loader2 className="h-6 w-6 animate-spin text-[#b98cff]" /></div> : <Tabs value={activeSection} onValueChange={setActiveSection} orientation="vertical" className="mt-8 grid items-start gap-6 lg:grid-cols-[220px_minmax(0,1fr)]"><TabsList className="sticky top-5 z-20 flex h-auto w-full flex-row flex-wrap justify-start gap-1 rounded-2xl border border-white/10 bg-[#121015] p-2 lg:flex-col lg:items-stretch"><TabsTrigger value="content" className="justify-start rounded-xl px-4 py-3 font-mono text-[10px] uppercase tracking-[.12em] text-white data-[state=active]:bg-[#b98cff] data-[state=active]:text-[#160e22]">Контент</TabsTrigger><button type="button" onClick={() => { setActiveSection("content"); window.setTimeout(() => document.getElementById("portfolio-editor")?.scrollIntoView({ behavior: "smooth", block: "start" }), 0); }} className="rounded-xl px-4 py-3 text-left font-mono text-[10px] uppercase tracking-[.12em] text-white transition hover:bg-white/10">Портфолио</button><TabsTrigger value="media" className="justify-start rounded-xl px-4 py-3 font-mono text-[10px] uppercase tracking-[.12em] text-white data-[state=active]:bg-[#b98cff] data-[state=active]:text-[#160e22]">Медиа</TabsTrigger><TabsTrigger value="submissions" className="justify-start rounded-xl px-4 py-3 text-left font-mono text-[10px] uppercase tracking-[.12em] text-white data-[state=active]:bg-[#b98cff] data-[state=active]:text-[#160e22]">Заявки{(submissionsQuery.data?.length ?? 0) > 0 ? ` · ${submissionsQuery.data?.length}` : ""}</TabsTrigger></TabsList>
