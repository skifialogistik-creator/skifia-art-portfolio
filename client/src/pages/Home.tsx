...
function ProjectStackCard({ index, total, work, ui }: { index: number; total: number; work: SiteContent["projects"][number]; ui: ReturnType<typeof getUiCopy> }) {
  ...
  return <div className="project-sticky h-[78vh] sm:h-[84vh]" style={{ top: `${96 + index * 28}px` }}><motion.article initial={{ opacity: 0, y: 34 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.24 }} transition={{ duration: 0.65, ease: [0.25, 0.1, 0.25, 1] }} className="project-sheet h-full" style={{ scale: targetScale }}><div className="flex flex-wrap items-start justify-between gap-4"><div className="flex min-w-0 items-center gap-4 sm:gap-7"><span className="font-display text-[clamp(3rem,8vw,7rem)] font-semibold leading-none tracking-[-.07em] text-[#d7e2ea]">{work.number}</span><div><p className="font-mono text-[10px] uppercase tracking-[0.17em] text-[#9faab4]">{work.category}</p><h3 className="mt-2 font-display text-2xl font-semibold tracking-[-.04em] text-white sm:text-4xl">{work.name}</h3></div></div>{work.url ? <a href={work.url} target="_blank" rel="noreferrer" className="live-button">{ui.storefrontOpen} <ExternalLink className="h-4 w-4" /></a> : <span className="live-button live-button--muted">{ui.storefrontRequestPlaceholder} <ArrowUpRight className="h-4 w-4" /></span>}</div><div className={`project-art project-art--${work.visual} mt-5 grid h-[calc(100%-130px)] grid-cols-[.7fr_1fr] gap-3 sm:mt-8`}><div className="grid grid-rows-2 gap-3">{caseScreenshots.length ? caseScreenshots.map((screenshot) => <figure key={screenshot.src} className="project-art__tile project-art__tile--screenshot"><img src={screenshot.src} alt={screenshot.alt} className={`project-case-screenshot project-case-screenshot--${screenshot.kind}`} loading="lazy" /><figcaption>{screenshot.label}</figcaption></figure>) : <><div className="project-art__tile project-art__tile--a"><span>{work.number}</span><b>{ui.decorativeDirection.split("\n").map((line) => <span key={line} className="block">{line}</span>)}</b></div><div className="project-art__tile project-art__tile--b"><i /><i /><i /></div></>}</div><div className={`project-art__tile project-art__tile--hero ${coverUrl ? "project-art__tile--hero-cover" : ""}`}>{coverUrl && <><img src={coverUrl} alt="Визуальная обложка кейса Skifia" className="project-case-cover" /><span className="project-case-cover-wash" aria-hidden="true" /></>}<em>{ui.decorativeFuture.split("\n").map((line) => <span key={line} className="block">{line}</span>)}</em>{isSkifiaCase && <p className="project-case-summary"><span>{ui.resultLabel} / 01</span>{work.description}</p>}<span className="project-art__orb" /><small>case / {String(index + 1).padStart(2, "0")}</small></div></div></motion.article></div>;
}

function StorefrontSiteCard({ work, index, onRequest, ui }: { work: SiteContent["projects"][number]; index: number; onRequest: (work: SiteContent["projects"][number]) => void; ui: ReturnType<typeof getUiCopy> }) {
