...
const serviceStats = [["01", "СТРАТЕГИЯ\nИ СМЫСЛ"], ["02", "ДИЗАЙН\nИ СИСТЕМА"], ["03", "КОД\nИ ЗАПУСК"]] as const;
const serviceWords = ["СМЕЛЫЕ", "САЙТЫ", "ПОД КЛЮЧ"];
...
function ServiceShowcase() {
  ...
  return <section id="services" className="services-showcase relative z-10 isolate min-h-screen overflow-hidden rounded-t-[42px] bg-white px-5 py-6 text-black sm:rounded-t-[60px] sm:px-8 sm:py-8 md:px-12">
    <video className="services-video pointer-events-none absolute inset-0 -z-20 h-full w-full object-cover" autoPlay loop muted playsInline preload="metadata" aria-hidden="true"><source src="/manus-storage/chrome-kinetic-reference_69492557.mp4" type="video/mp4" /></video>
    ...
    <div className="mx-auto flex min-h-[calc(100svh-3rem)] max-w-[1600px] flex-col sm:min-h-[calc(100svh-4rem)]">
      ...
      <div className="grid gap-7 pb-4 sm:gap-10 md:grid-cols-[minmax(180px,.52fr)_1fr] md:items-end md:pb-7"><div className="flex justify-between gap-4 md:flex-col"><motion.p initial={reduced ? false : { opacity: 0, y: 18, filter: "blur(5px)" }} whileInView={reduced ? undefined : { opacity: 1, y: 0, filter: "blur(0px)" }} viewport={{ once: true, amount: 0.25 }} transition={{ delay: 0.58, duration: 0.62, ease: serviceEase }} className="services-caption services-caption--primary">Структура, визуал и разработка — чтобы сайт точно передавал характер вашего проекта.</motion.p><motion.p {...fadeUp(0.72)} className="self-end font-mono text-[9px] font-semibold uppercase tracking-[.16em] text-[#5e0ed7]">03 этапа / один результат</motion.p></div><div className="flex items-end justify-between gap-4"><motion.p initial={reduced ? false : { opacity: 0, y: 18, filter: "blur(5px)" }} whileInView={reduced ? undefined : { opacity: 1, y: 0, filter: "blur(0px)" }} viewport={{ once: true, amount: 0.25 }} transition={{ delay: 0.78, duration: 0.62, ease: serviceEase }} className="services-caption services-caption--secondary">От первого смысла и прототипа — до запущенного сайта, которым легко управлять.</motion.p><h2 className="text-right font-display text-[clamp(2.7rem,8.7vw,8.8rem)] font-semibold uppercase leading-[.79] tracking-[-.1em]">{serviceWords.map((word, index) => <span key={word} className="services-word block overflow-hidden"><motion.span initial={reduced ? false : { y: "112%" }} whileInView={reduced ? undefined : { y: 0 }} viewport={{ once: true, amount: 0.25 }} transition={{ delay: 0.38 + index * 0.14, duration: 0.7, ease: serviceEase }} className="block">{word}</motion.span></span>)}</h2></div></div>
    </div>
  </section>;
}
...
export default function Home() {
  ...
  return <div className="creator-shell min-h-screen overflow-x-clip bg-[#0c0c0c] text-[#d7e2ea]">
    ...
    <main>
      <section id="top" className="relative flex min-h-screen flex-col overflow-hidden"><div className="hero-noise pointer-events-none absolute inset-0" /><div className="hero-glow pointer-events-none absolute left-1/2 top-[38%] h-[42vw] w-[42vw] min-h-64 min-w-64 -translate-x-1/2 -translate-y-1/2 rounded-full" /><MagneticAvatar /><div className="pointer-events-none relative z-20 mx-auto flex w-full max-w-[1600px] flex-1 flex-col px-6 pt-32 sm:px-10 sm:pt-40"><FadeIn className="pointer-events-auto overflow-hidden" delay={0.12}><h1 className="hero-heading whitespace-nowrap font-display text-[5.75vw] font-black uppercase leading-[.84] tracking-[-0.1em] sm:text-[6.25vw] md:text-[6.5vw] lg:text-[7vw]">Привет,<br />я <span>создаю</span><br />сайты</h1></FadeIn><div className="mt-auto flex items-end justify-between gap-5 pb-7 sm:pb-10"><FadeIn delay={0.3} className="pointer-events-auto relative z-20 max-w-[160px] text-[clamp(.72rem,1.4vw,1.25rem)] font-light uppercase leading-snug tracking-wide text-[#d7e2ea] sm:max-w-[260px]">Дизайн и разработка сайтов, которые хочется рассматривать и открывать снова.</FadeIn><FadeIn delay={0.45} className="pointer-events-auto relative z-20"><a href="#brief" className="contact-button">Обсудить проект <ArrowDownRight className="h-4 w-4" /></a></FadeIn></div></div></section>
