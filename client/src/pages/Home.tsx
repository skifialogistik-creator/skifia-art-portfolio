...
function ServiceShowcase({ content, videoUrl }: { content: SiteContent; videoUrl: string }) {
  ...
  return <section id="services" className="services-showcase relative z-10 isolate min-h-screen overflow-hidden rounded-t-[42px] bg-[#cbb5f0] px-5 py-6 text-[#201529] sm:rounded-t-[60px] sm:px-8 sm:py-8 md:px-12">
    ...
    <div className="mx-auto flex min-h-[calc(100svh-3rem)] max-w-[1600px] flex-col sm:min-h-[calc(100svh-4rem)]">
      ...
      <div className="grid gap-7 pb-4 sm:gap-10 md:grid-cols-[minmax(320px,.72fr)_1fr] md:items-end md:pb-7"><div className="services-annotation-grid">{serviceNotes.map((note, index) => <motion.article key={`service-note-${index}`} initial={reduced ? false : { opacity: 0, y: 24, scale: 0.97 }} whileInView={reduced ? undefined : { opacity: 1, y: 0, scale: 1 }} whileHover={reduced ? undefined : { y: -6, scale: 1.015 }} viewport={{ once: true, amount: 0.22 }} transition={{ delay: 0.56 + index * 0.1, duration: 0.52, ease: serviceEase }} className="services-annotation-card"><span>{String(index + 1).padStart(2, "0")}</span><p>{note}</p><i aria-hidden="true" /></motion.article>)}</div><div className="flex items-end justify-end"><h2 className="text-right font-display text-[clamp(2.7rem,8.7vw,8.8rem)] font-semibold leading-[.77] tracking-[-.07em]">{serviceWords.map((word, index) => <span key={`${word}-${index}`} className="services-word block overflow-hidden"><motion.span initial={reduced ? false : { y: "112%" }} whileInView={reduced ? undefined : { y: 0 }} viewport={{ once: true, amount: 0.25 }} transition={{ delay: 0.38 + index * 0.14, duration: 0.7, ease: serviceEase }} className="block">{word}</motion.span></span>)}</h2></div></div>
    </div>
  </section>;
}
