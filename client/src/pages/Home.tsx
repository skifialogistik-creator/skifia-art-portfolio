...
function RotatingHeroPhrase({ fallback }: { fallback: string }) {
  const reduced = useReducedMotion();
  const phrases = useMemo(() => [fallback, ...heroRotatingPhrases.filter((phrase) => phrase !== fallback)], [fallback]);
...
function ProjectStackCard({ index, total, work }: { index: number; total: number; work: SiteContent["projects"][number] }) {
  ...
  const isSkifiaCase = index === 0 && Boolean(work.url);
  const coverUrl = null;
  const caseScreenshots: Array<{ src: string; alt: string; label: string; kind: "hero" | "services" }> = [];

  return <div className="project-sticky h-[78vh] sm:h-[84vh]" style={{ top: `${96 + index * 28}px` }}><motion.article initial={{ opacity: 0, y: 34 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.24 }} transition={{ duration: 0.65, ease: [0.25, 0.1, 0.25, 1] }} className="project-sheet h-full" style={{ scale: targetScale }}><div className="flex flex-wrap items-start justify-between gap-4"><div className="flex min-w-0 items-center gap-4 sm:gap-7"><span className="font-display text-[clamp(3rem,8vw,7rem)] font-semibold leading-none tracking-[-.07em] text-[#d7e2ea]">{work.number}</span><div><p className="font-mono text-[10px] uppercase tracking-[0.17em] text-[#9faab4]">{work.category}</p><h3 className="mt-2 font-display text-2xl font-semibold tracking-[-.04em] text-white sm:text-4xl">{work.name}</h3></div></div>{work.url ? <a href={work.url} target="_blank" rel="noreferrer" className="live-button">Открыть сайт <ExternalLink className="h-4 w-4" /></a> : <span className="live-button live-button--muted">Ссылка появится <ArrowUpRight className="h-4 w-4" /></span>}</div><div className={`project-art project-art--${work.visual} mt-5 grid h-[calc(100%-130px)] grid-cols-[.7fr_1fr] gap-3 sm:mt-8`}><div className="grid grid-rows-2 gap-3">{caseScreenshots.length ? caseScreenshots.map((screenshot) => <figure key={screenshot.src} className="project-art__tile project-art__tile--screenshot"><img src={screenshot.src} alt={screenshot.alt} className={`project-case-screenshot project-case-screenshot--${screenshot.kind}`} loading="lazy" /><figcaption>{screenshot.label}</figcaption></figure>) : <><div className="project-art__tile project-art__tile--a"><span>{work.number}</span><b>visual<br />direction</b></div><div className="project-art__tile project-art__tile--b"><i /><i /><i /></div></>}</div><div className={`project-art__tile project-art__tile--hero ${coverUrl ? "project-art__tile--hero-cover" : ""}`}>{coverUrl && <><img src={coverUrl} alt="Визуальная обложка кейса Skifia" className="project-case-cover" /><span className="project-case-cover-wash" aria-hidden="true" /></>}<em>future<br />on<br />screen</em>{isSkifiaCase && <p className="project-case-summary"><span>результат / 01</span>{work.description}</p>}<span className="project-art__orb" /><small>case / {String(index + 1).padStart(2, "0")}</small></div></div></motion.article></div>;
}

function StorefrontSiteCard({ work, index, onRequest }: { work: SiteContent["projects"][number]; index: number; onRequest: (work: SiteContent["projects"][number]) => void }) {
  const [flipped, setFlipped] = useState(false);
  const coverUrl = work.coverUrl;
  const isSold = work.availability === "sold";
  const availabilityLabel = isSold ? "Продан" : "В продаже";
  const hasUrl = Boolean(work.url) && !isSold;
  const hostLabel = work.url ? work.url.replace(/^https?:\/\/(www\.)?/, "").split("/")[0] : "ваш-сайт.ru";
...
export default function Home() {
  ...
  const rotatesHeroPhrase = content.hero.lineTwo.includes("создаю") || content.hero.lineThree.includes("создаю");
