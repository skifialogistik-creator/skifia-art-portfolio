...
## Чистый запуск из GitHub

Если прежний проект и его данные недоступны, создайте воспроизводимый seed из закоммиченного контента командой:

```bash
pnpm run seed:github-defaults
```

Команда создаёт локальные файлы `cloudflare/backup/github-default-content.json` и `cloudflare/backup/github-default-content.sql`. В seed входят тексты, контакты, структура и описание кейса из `shared/siteContent.ts`; заявки, Telegram-настройки и утраченные медиафайлы намеренно не создаются. Старые URL `manus-storage` отключены, поэтому новый сайт не будет обращаться к удалённому прежнему хранилищу. После запуска загрузите актуальный портрет, два видеоролика и обложки через защищённую админ-панель.

## Сохранение данных

Если старые данные всё же будут найдены вне Git-репозитория, сначала необходим экспорт в JSON. Файл должен быть сохранён как `cloudflare/backup/legacy-export.json` и иметь следующий формат:
